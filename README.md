...
`--name-from-content`는 새 노트를 `핵심 주제.md`로 저장하며, 파일명과 중복되는 본문 `# 제목`은 넣지 않습니다. 원본 파일은 바꾸지 않습니다. 생략하면 기존 `원본명.md` 규칙과 본문 제목을 유지합니다. 제목이 없으면 원본명을 사용합니다. 제목의 경로 구분자·파일명 금지 문자·Obsidian 링크 특수문자를 정리하고 UTF-8 파일명 길이를 제한합니다. 이미 생성된 노트는 링크 보존을 위해 자동으로 이름을 바꾸지 않습니다.
